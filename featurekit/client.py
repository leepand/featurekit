...
def get_week_day():
    utc_now = datetime.utcnow().replace(tzinfo=timezone.utc) - timedelta(hours=12)
...
class FeatureSDK:
    ...
    def __init__(
        self, host="http://172.31.25.101:5006", db_name="", model_name="", data_path=""
    ):
        """
        初始化FeatureSDK对象。

        参数：
        - host (str)：SDK服务器的基本URL。默认为"http://1.31.24.138:6901"。CF: http://172.31.25.101:5006
        - db_name (str): 数据库的名称。
        ...
        self.center_db_name = "monitor.db"
    ...
    def push_feature_from_db(
        self, correct_data_cnt, mode="dev", model_name="feature_store", version=0.0
    ):
        """
        ...
        self.db_store.set(info_key, pickle.dumps(data))
